...
Флаг `--site ИМЯ` заливает на конкретный сайт вместо `settings.SITE_NAME`, не
трогая сам файл — нужен для `MyDocking_parallel.py` (см. ниже) и вообще для
разовой заливки на сайт, на который переключаться насовсем не собираешься.
...
### Докинг сразу на нескольких сайтах

`MyDocking_parallel.py` делит `LOCAL_SDF` между сайтами из `PARALLEL_SITES`
(в `settings.py`) пропорционально их `cpu_count` и отправляет отдельную
задачу на каждый — параллельно, а не по очереди. У SKIF_GRID_CIS и SKIF_GEO
разные файловые системы, поэтому задачи независимы; на кластере имя
`dockscore.csv` не зависит от имени SDF, поэтому результаты каждого сайта
скачиваются в свою подпапку `results/<дата_время>/<SITE_NAME>/`, а не в общую —
иначе один сайт затёр бы результат другого при скачивании. По завершении обоих
`dockscore_combined.csv` в корне папки запуска — просто конкатенация с
пересортировкой по `E_vina, kcal/mol`.

Перед первым запуском на новом для себя сайте: `python tools/sync_cluster.py --site ИМЯ`
— как и при обычном переключении `SITE_NAME`, код сам на кластер не попадёт.

```powershell
python MyDocking_parallel.py
```

Тот же функционал есть в GUI — стадия «Докинг на двух кластерах»
(`gui/schema.py`: `dock_cluster_parallel`).
