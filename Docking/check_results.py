...
def print_progress(job_id_file):
    ...
    if p["total"]:
        ...
        reason = ("кластер не ответил на запрос общего числа — попробуй ещё раз"
                  if p["timed_out"] else
                  "на кластере ещё не закончилась подготовка PDBQT")
        print(f"📈 Прогресс: задокировано уже {p['done']} ({reason})")
    elif p["timed_out"]:
        print("⚠️  Кластер не ответил вовремя ни на один из запросов — попробуй ещё раз позже.")
    else:
